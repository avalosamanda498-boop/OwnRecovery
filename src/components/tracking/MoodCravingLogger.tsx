...
export default function MoodCravingLogger({ user, roleCopy }: MoodCravingLoggerProps) {
  const [selectedMood, setSelectedMood] = useState<MoodOption | null>(null)
  const [selectedCraving, setSelectedCraving] = useState<CravingOption | null>(null)
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!selectedMood || !selectedCraving) return
  ...
  const buttonDisabled = !selectedMood || !selectedCraving || submitting
  ...
  return (
    <section className="bg-white border border-gray-200 rounded-2xl shadow-sm p-6 space-y-6">
      ...
      <form className="space-y-6" onSubmit={handleSubmit}>
        ...
        <div className="space-y-3">
          <h3 className="text-sm font-medium text-gray-700">Craving check-in</h3>
      ...
      {latestEntry && (
        <div className="rounded-xl border border-gray-200 p-4 bg-gray-50 text-sm text-gray-700">
          ...
          <p className="mt-1">
            Craving: <span className="font-medium capitalize">{latestEntry.craving_level.replace('_', ' ')}</span>
          </p>
